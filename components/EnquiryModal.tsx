...
export const EnquiryModal: React.FC<EnquiryModalProps> = ({ isOpen, onClose, title = "Enquire Now" }) => {
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center px-4">
            <div className="absolute inset-0 bg-navy-950/95 backdrop-blur-sm" onClick={onClose} />
            <div className="relative bg-navy-900 border border-white/10 w-full max-w-lg overflow-hidden rounded-3xl shadow-2xl animate-in fade-in zoom-in duration-300">
                <div className="pt-8 pb-4 px-6 flex flex-col items-center relative border-b border-white/5">
                    <button onClick={onClose} className="absolute right-6 top-6 text-gray-400 hover:text-white transition-colors cursor-pointer">
                        <X size={20} />
                    </button>
                    <p className="text-gold-400 font-mono text-[9px] uppercase tracking-[0.3em] mb-1">Neoliv Plotted Living</p>
                    <h3 className="text-white font-serif text-xl tracking-[0.15em] uppercase">Grand Forest Privé</h3>
                ...
                <div className="p-8 pt-6 min-h-[350px] flex flex-col justify-center">
                    {isLocalSubmitted ? (
                        <div className="text-center py-8 animate-in fade-in zoom-in duration-500">
                            <div className="w-20 h-20 bg-gold-400/10 text-gold-400 rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-gold-400/20 border border-gold-400/30">
                                <CheckCircle2 size={36} />
                            </div>
                            <h3 className="text-white font-serif text-2xl tracking-[0.1em] mb-3 uppercase">Thank You</h3>
                            <p className="text-gray-300 leading-relaxed text-sm max-w-xs mx-auto font-light">
                                Our luxury property advisors have received your request for Grand Forest Privé and will reach out to you shortly.
                            </p>
                            <button onClick={onClose} className="mt-8 px-10 py-3.5 bg-gold-400 text-navy-950 rounded-full hover:brightness-110 transition-all text-xs tracking-widest uppercase font-bold cursor-pointer">
                                Close Window
                            </button>
                        </div>
                    ) : (
                        <>
                            <p className="text-gray-400 text-[11px] uppercase tracking-widest mb-6 text-center">
                                Connect with our luxury property advisors
                            </p>
                            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); handleSubmit('callback'); }}>
                                ...
                                <div className="space-y-1">
                                    <input
                                        type="tel" name="phone" ref={phoneInputRef} value={formData.phone} onChange={handleInputChange}
                                ...
                                <div className="pt-1">
                                    <label className="flex items-start gap-2.5 cursor-pointer select-none">
                                        ...
                                        />
                                        <span className="text-[10px] text-gray-300 leading-relaxed font-light">
